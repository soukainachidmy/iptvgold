      ...
      <section className="hero-gradient pt-32 pb-12 lg:pt-40 lg:pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <Link href="/installation-guide" className="hover:text-[#3b8ef3]">Installation Guides</Link>
            <span>/</span>
            <span className="text-gray-300">{guide.device}</span>
          </nav>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold mb-4">
            {guide.title}
          </h1>
          <p className="text-lg text-gray-400">{guide.intro}</p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-[1fr_280px] gap-12">
            <div className="prose-iptv">
                    ...
                    <ol className="space-y-3 mb-6">
                      {section.steps.map((step, j) => (
                        <li key={j} className="flex gap-3 text-gray-300">
                          <span className="flex-shrink-0 w-7 h-7 rounded-full bg-[#1a6fde]/20 border border-[#1a6fde]/40 flex items-center justify-center text-sm font-bold text-[#3b8ef3]">
                            {j + 1}
                          </span>
              ...
              <div className="mt-8 p-6 rounded-2xl bg-gradient-to-br from-[#0d1a3a] to-[#0f2247] border border-[#1a6fde]/30">
                <h3 className="text-xl font-bold text-white mb-2">Ready to Start Streaming?</h3>
                <p className="text-gray-300 mb-4">Get your IPTVGOLD subscription and follow this guide to set up in minutes.</p>
            ...
            <aside className="hidden lg:block">
              <div className="sticky top-24">
                <div className="bg-[#111118] border border-[#1e1e2e] rounded-2xl p-6 mb-6">
                  <h3 className="text-sm font-semibold text-white uppercase tracking-wider mb-4">Other Guides</h3>
                  <ul className="space-y-3">
                    {otherGuides.map((g) => (
                      <li key={g.slug}>
                        <Link href={`/installation-guide/${g.slug}`} className="text-sm text-gray-400 hover:text-[#3b8ef3] transition-colors">
                          {g.device}
                        </Link>
                      </li>
                    ))}
                ...
                <div className="bg-[#111118] border border-[#1e1e2e] rounded-2xl p-6">
                  <h3 className="text-sm font-semibold text-white uppercase tracking-wider mb-4">Need Help?</h3>
                  <p className="text-sm text-gray-400 mb-4">Our support team is available 24/7 to help with installation.</p>
                  <a href="mailto:support@iptvgold.com" className="text-sm font-semibold text-[#3b8ef3] hover:text-[#5aa3ff]">
                    Contact Support →
                  </a>
                </div>
              </div>
